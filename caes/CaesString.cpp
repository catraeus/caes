...
const static char  *oomPrefix    = (const char *)"afpnumRkMGTPY";      // atto, femto yada yada yada Peta
const static char  *oomHexLookup = (const char *)"0123456789ABCDEF";  // Fast and furious, no c format overhead


enum  eSciEng        {ESE_NUM,    ESE_ENG,   ESE_SCI                                 };
enum  eStrEngState   {ESES_START, ESES_MANT, ESES_EXP, ESES_DONE, ESES_RES, ESES_BAD };
...
/*
  Given the unknown string, i_s and the reference string, i_f.  Convolve
    ...
    For a match, the output is a pointer to the location where found.
    For no match, the output is NULL.
...
*/
  ...
  while(iOuter < nTries) {
    ...
    oops = false;
    while((*pRef != '\0') && (!oops)) {
      if(*(pSrc++) != *(pRef++)) oops = true;
    }
    if(oops) {
      iOuter++;  // This will just bomb out with o_pFound being NULL
    }
    else {
      o_pFound = &(i_s[iOuter]);
      iOuter = nTries; // Hooray, exit and say where we found it.
    }
  }
...
bool   StringEng             (char *i_tStr, double *o_f) {
  ...
  if(strlen(pSrc) == 0)
    return false;
  while((state != ESES_DONE) && (state != ESES_RES) && (state != ESES_BAD)) {
    if(((*pSrc <= '9') && (*pSrc >= '0')) || (*pSrc == '-') || (*pSrc == '+')) {
      if(state == ESES_START) // Otherwise, the rest of the state machine just remains intact.
        state = ESES_MANT;
      *(pDst++) = *(pSrc++);
      }
    else {
      if(state == ESES_START) {
        if(ParseEngMark(*pSrc) < 99) {
          ...
          *(pDst++) = '.';
          ...
          fprintf(stderr, "StringEng() Non Eng characters at start:  %s\n", pSrc);fflush(stderr);
          state = ESES_BAD;
        }
      }
      else if(state == ESES_EXP) { // Stuff after the exponent numbers, call it residue.
        *pDst = '\0';
        if(strlen(pSrc) > 0) {
          state = ESES_RES;
          fprintf(stderr, "StringEng() post-exp residue:  %s\n", pSrc);fflush(stderr);
        }
        else {
          state = ESES_DONE;
        }
        ...
        if((*pSrc == 'e') || (*pSrc == 'E')) {
          if(gotEngMark) { // Yes, it's an exponent char, but we already had one
            *pDst = '\0';
            fprintf(stderr, "StringEng() extra-exp residue E:  %s\n", pSrc);fflush(stderr);
            state = ESES_RES;
            }
          else{ // True exponent, let's see if there are numbers next.
            state = ESES_EXP;
            sciEng = ESE_SCI;
            pSrc++;
            *pDst = '\0';
          ...
          if(gotMantPeriod) { // Extraneous, treat as residue.
            *pDst = '\0';
            fprintf(stderr, "StringEng() extra-exp residue .:  %s\n", pSrc);fflush(stderr);
            state = ESES_RES;
            }
          else  { // keep going in the mantissa for a period
            *(pDst++) = *(pSrc++);
            gotMantPeriod = true;
            }
          }
          ...
          if(sciEng == ESE_ENG) {
            *pDst = '\0';
            fprintf(stderr, "StringEng() second eng mark, treating as residue:  %s\n", pSrc);fflush(stderr);
            state = ESES_RES;
          }
          else {
            iExp = ParseEngMark(*pSrc);
          ...
          fprintf(stderr, "StringEng() random trailing characters:  %s\n", pSrc);fflush(stderr);
          state = ESES_DONE;
          }
        }
      }
